...
tid_t process_execute(const char *file_name)
{
  ...
  sema_down(&t->sema_exec_child);
  if (t->child_exec_success == 0)
    return -1;
  return tid;
}
...
start_process(void *file_name_)
{
  ...
  strlcpy(file_name_cpy, file_name_, PGSIZE);
  char *save_ptr;
  char *proc_name = strtok_r(file_name, " ", &save_ptr);
  ...
  if (!success)
  {
    t->exit_code = -1;
    t->info->is_alive = 0;
    t->parent->child_exec_success = 0;
    sema_up(&t->parent->sema_exec_child);
    thread_exit();
  }
  t->parent->child_exec_success = 1;
  sema_down(&file_sema);
  struct file *open_file = filesys_open(proc_name);
  thread_current()->executing_file = open_file;
  ...
  //   esp -= esp % 4;
  // if_.esp = (void *)esp;
  if ((unsigned)(if_.esp) % 4 != 0)
    if_.esp -= (unsigned)if_.esp % 4;
  size_t ptr_size = sizeof(char *);
  push(&if_.esp, NULL, ptr_size);
  for (int i = argc - 1; i >= 0; i--)
    push(&if_.esp, &argv[i], ptr_size);
  if_.esp -= ptr_size;
  *(uintptr_t *)if_.esp = ((uintptr_t)if_.esp + ptr_size);
  push(&if_.esp, &argc, ptr_size);
  // if_.esp -= ptr_size;
  // *(int *)if_.esp = argc;
  push(&if_.esp, NULL, ptr_size);
...
int process_wait(tid_t child_tid)
{
       ...
       e = list_next(e))
  {
    ...
    {
      if (entry->been_waited)
      {
        return -1;
      }
      else if (entry->is_alive == 0)
      {
