...
process_execute (const char *file_name) 
{
  ...
  /* Make a copy of FILE_NAME.
     ...
     通过深拷贝来避免竞争。
     注意！！此处请使用 palloc_get_page 而非 malloc。
     */
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if (vm_insert_file_spte(cur->spage_table,
        upage,file,ofs,page_read_bytes,page_zero_bytes,writable)==false)
        return false;
#else
      /* Get a page of memory. */
      uint8_t *kpage = vm_frame_alloc(PAL_USER,upage);//palloc_get_page (PAL_USER);
      if (kpage == NULL)
        return false;
...
setup_stack (void **esp) 
{
  ...
  kpage = vm_frame_alloc(PAL_USER|PAL_ZERO,PHYS_BASE-PGSIZE);//palloc_get_page (PAL_USER | PAL_ZERO);
  if (kpage != NULL) 
    {
...
#ifdef VM
  install_success=install_success&&vm_instert_frame_spte(t->spage_table,upage,kpage);
  if(install_success)
